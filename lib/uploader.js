...
const uploader = {
  ...
  queueWorker: (params, cb) => {
    glacier.uploadMultipartPart(params, (err, result) => {
      ...
      cb();
      return uploader.uploadDone(err, params, result, cb);
    });
  },
  ...
  upload: (file, vaultName, region, concurency) => {
    ...
    glacier.initiateMultipartUpload(initParams, (err, { uploadId }) => {
      ...
      uploader.fileStream = fs.createReadStream(filePath, { highWaterMark: partSize })
        .on('data', (body) => {
          ...
          uploader.requestCount += 1;
          if (uploader.requestCount > concurency && !uploader.fileStream.isPaused()) {
  ...
  uploadRetry: (delay, params) => {
    debug('Retry scheduled: %s, seconds', delay);
    uploader.retries += 1;
    setTimeout(() => {
      glacier.uploadMultipartPart(params, (err, result) => {
        if (err) {
        ...
        uploader.uploadDone(err, params, result);
        return true;
      });
    }, (delay + 1) * 1000);
  },
